...
const PDF_PATH = path.join(process.cwd(), "public", "3S_propo.pdf");
...
function readAnalytics(): Analytics {
  try {
    ...
    return { pageViews: {}, catalogDownloads: [], proposalDownloads: [] };
  }
}
...
export async function GET(req: NextRequest) {
  ...
  if (!BOT_RE.test(ua)) {
    try {
      ...
      data.proposalDownloads = increment(data.proposalDownloads, date);
      writeAnalytics(data);
    } catch (err) {
      console.error("[proposal/download] analytics 기록 실패:", err);
    }
  }
  ...
  try {
    ...
    return new NextResponse(buffer, {
      headers: {
        ...
        "Content-Disposition": 'attachment; filename="emersia-proposal.pdf"',
